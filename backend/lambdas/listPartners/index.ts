...
import { listPartnerProfiles } from '../../shared/partners';
...
const baseHandler: APIGatewayProxyHandler = async (event) => {
  try {
    ...
    const partners = await listPartnerProfiles(athleteId);
    return response(200, {
      partners: coachMode ? partners.filter((item) => item.visibility === 'shared-with-coach') : partners
    });
  } catch (error) {
...
export const handler: APIGatewayProxyHandler = withRequestLogging('listPartners', baseHandler);
