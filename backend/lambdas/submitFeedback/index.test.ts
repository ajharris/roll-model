...
describe('submitFeedback handler', () => {
  ...
  it('creates a GitHub issue', async () => {
    const result = (await handler(
      buildEvent(validPayload, { sub: 'user-1', 'custom:role': 'athlete', email: 'user@example.com' }),
      {} as never,
      () => undefined,
  ...
  it('rejects invalid payloads', async () => {
    const result = (await handler(
      buildEvent({ ...validPayload, type: 'unknown' }, { sub: 'user-1', 'custom:role': 'athlete' }),
      {} as never,
      () => undefined,
    )) as APIGatewayProxyResult;
  ...
  it('rejects submission without preview confirmation', async () => {
    const result = (await handler(
      buildEvent({ ...validPayload, previewConfirmed: false }, { sub: 'user-1', 'custom:role': 'athlete' }),
      {} as never,
      () => undefined,
    )) as APIGatewayProxyResult;
